...
def test_execution_mode_helpers_shadow_mode():
    s = Settings(execution_mode="shadow_live", paper_trading_mode=False)
    assert s.is_simulated_execution() is True
    assert s.is_shadow_live_execution() is True


def test_execution_mode_helpers_legacy_paper_flag():
    s = Settings(execution_mode="live", paper_trading_mode=True)
    assert s.normalized_execution_mode() == "paper"
