...
# Optional RL execution policy for rest/chase/cancel decisions
EXECUTION_RL_ENABLED=false
# dqn (tabular Q-learning proxy) or ppo (reserved for external policy integration)
EXECUTION_RL_ALGO=dqn
EXECUTION_RL_LEARNING_RATE=0.03
EXECUTION_RL_DISCOUNT=0.95
EXECUTION_RL_EPSILON=0.15
EXECUTION_RL_MIN_EPSILON=0.02
EXECUTION_RL_EPSILON_DECAY=0.999
EXECUTION_RL_CHASE_STEP_BPS=1.5
EXECUTION_RL_MAX_CHASE_BPS=12
EXECUTION_RL_UPDATE_INTERVAL_SECONDS=2

BASE_ORDER_NOTIONAL_USD=10

# Safe-start inventory bootstrap behavior
SAFE_START_ENABLED=true
BASE_BUY_MODE=off
# market (single IOC), twap (time-sliced IOC), or vwap (volume-profiled slices)
BASE_BUY_EXECUTION_ALGO=market
BASE_BUY_EXECUTION_SLICES=6
BASE_BUY_EXECUTION_WINDOW_SECONDS=120
BASE_BUY_VWAP_LOOKBACK_CANDLES=24
BASE_BUY_VWAP_GRANULARITY=FIVE_MINUTE
# Allocation refresh cadence + multiplier guardrails
KELLY_ALLOCATION_ENABLED=true
KELLY_REFRESH_SECONDS=900
KELLY_MIN_ALLOCATION_FRAC=0.25
KELLY_MAX_ALLOCATION_FRAC=2.50
# Black-Litterman blend controls (equilibrium + HMM regime views)
BLACK_LITTERMAN_TAU=0.05
BLACK_LITTERMAN_RISK_AVERSION=2.5
BLACK_LITTERMAN_CONFIDENCE_FLOOR=0.05
BLACK_LITTERMAN_VIEW_RETURN_ABS=0.01
